...
def connect():
    ...
    #DONE: 1 of 6 fill out MySQL connection parameters. 
...
def createUser(username, password):
    ...
    #DONE 2 of 6. Use cur.execute() to insert a new row into the users table containing the username, salt, and passwordhash
    cur.execute(
        "INSERT INTO users (username, salt, passwordhash) VALUES (%s, %s, %s);",
...
def validateUser(username, password):
    ...
    #DONE 3 of 6. Use cur.execute() to select the appropriate user record (if it exists)
    cur.execute(
...
def fetchUser(username):
    ...
    #DONE 4 of 6. Use cur.execute() to fetch the row with this username from the users table, if it exists
    cur.execute(
        "SELECT id, username FROM users WHERE username = %s;", # unclear if supposed to get (id, username) pair or entire row
        (username,)
    )
...
def addHistory(user_id, query):
    ...
    #DONE 5 of 6. Use cur.execute() to add a row to the history table containing the correct user_id and query
    cur.execute(
...
def getHistory(user_id):
    ...
    #DONE 6 of 6. Use cur.execute() to fetch the most recent 15 queries from this user (including duplicates). 
